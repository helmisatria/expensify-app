...
test('should set startDate filter', () => {
  const state = filtersReducer(undefined, { type: 'SET_START_DATE', startDate: moment(0).subtract(4, 'days').valueOf() });
  expect(state.startDate).toBe(moment(0).subtract(4, 'days').valueOf());
});

test('should set endDate filter', () => {
  const state = filtersReducer(undefined, { type: 'SET_END_DATE', endDate: moment(0).add(4, 'days').valueOf() });
